"""슬롯 1~10 UI · 불러오기/저장/비우기 · Ctrl+1~0 선택."""
...
class SlotsUIMixin:
    """MacroStudioApp 에 믹스인 — 슬롯 목록/로드/저장/비우기."""

    def _build_left_slot_panel(self, parent: ctk.CTkFrame) -> None:
        parent.grid_propagate(False)
        parent.grid_rowconfigure(2, weight=1)

        ctk.CTkLabel(parent, text="매크로 슬롯 (1~10)", font=ctk.CTkFont(size=16, weight="bold")).grid(
            row=0, column=0, padx=12, pady=(12, 4), sticky="w"
        )
        ctk.CTkLabel(
            parent,
            text=f"저장: {storage.MACROS_DIR}/slot_XX.json\nCtrl+1~0 으로도 선택",
        ...
        ctk.CTkButton(btn_row, text="슬롯 불러오기", command=self._load_selected_slot, height=32).grid(
            row=0, column=0, columnspan=2, padx=4, pady=2, sticky="ew"
        )
        ctk.CTkButton(
            btn_row, text="슬롯 비우기", command=self._clear_selected_slot, height=32, fg_color="#a33", hover_color="#822"
        ).grid(row=1, column=0, columnspan=2, padx=4, pady=2, sticky="ew")
        ctk.CTkButton(btn_row, text="목록 새로고침", command=self._refresh_slot_list, height=28).grid(
            row=2, column=0, columnspan=2, padx=4, pady=2, sticky="ew"
        )

    def _select_slot_hotkey(self, slot: int) -> None:
    ...
    def _refresh_slot_list(self, select: int | None = None) -> None:
        ...
        for n, label, _doc in storage.list_slot_summaries():
            self.slot_listbox.insert(tk.END, label)
        if 1 <= keep <= storage.SLOT_COUNT:
            idx = keep - 1
    ...
    def _clear_selected_slot(self) -> None:
        ...
        if not messagebox.askyesno("슬롯 비우기", f"{slot}번 슬롯 파일을 삭제(비우기)할까요?"):
            return
        try:
